...
//                    RxNjjBleManger.getInstance().getAlarmClockInfo()
    ...
    fun parserOTAStart(byteArray: ByteArray, njjPushOtaCallback: NjjPushOtaCallback) {
        ...
        when (byteArray[4].toInt() and (0xff)) {
            0x00 -> {//自定义表盘
                LogUtil.e("开始写入自定义表盘")
                njjPushOtaCallback.onPushStart(0x00)
            }
            //表盘校验成功，开始推送表盘数据
            0x01 -> {//表盘
                LogUtil.e("开始写入表盘")
                njjPushOtaCallback.onPushStart(0x01)
            }

            0x02 -> {//联系人
                LogUtil.e("开始写入联系人")
                njjPushOtaCallback.onPushStart(0x02)
            }

            0x03 -> {//固件UI升级
                LogUtil.e("开始UI表盘")
                njjPushOtaCallback.onPushStart( 0x03)
            }
            0x04->{
                LogUtil.e("开始写入游戏")
                njjPushOtaCallback.onPushStart( 0x04)
            }
            0x05->{  //核酸码
                LogUtil.e("开始写入联系人")
                njjPushOtaCallback.onPushStart( 0x05)
            }
            0x09->{  //字体
                LogUtil.e("开始写入字体")
                njjPushOtaCallback.onPushStart(0x09)
            }
            0x0A->{
                LogUtil.e("开始写入杰里二代ota")
                njjPushOtaCallback.onPushStart(0x0A)
            }
        }
