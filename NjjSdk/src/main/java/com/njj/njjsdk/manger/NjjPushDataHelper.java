...
//                                pushListener.onPushStart();
    ...
    private void handPushSuccess() {
        NjjBleManger.getInstance().getBluetoothClient().clearRequest(NjjBleManger.getInstance()
                .getMac(), Constants.REQUEST_WRITE);
        destroyTimer();
...
//    8                       //本机推送失败
    private void handPushError(int data, int pack) {
        if (data == 1) {   //补包
            NjjBleManger.getInstance().getBluetoothClient().clearRequest(NjjBleManger.getInstance().getMac(), Constants.REQUEST_WRITE);
            isAdd = 1;
            mPkg = pack;
            initTimer(0,500);
        } else if (data == 6) {
    ...
    private void startPushData() {
        byte[] bytes = CmdMergeImpl.INSTANCE.sendDialData(isAdd, mPkg);
        if (null != bytes) {
...
//        LogUtil.d("初始化自定义表盘完毕");
    ...
    private void checkRes(String path) throws IOException {
        ...
        //startPushDial();
        LogUtil.e("--------------------表盘数据初始化完毕:" + buffer.length);
    }
