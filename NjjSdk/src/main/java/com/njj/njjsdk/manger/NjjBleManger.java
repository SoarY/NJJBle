...
import com.njj.njjsdk.utils.BLESDKLocalData;
...
import com.njj.njjsdk.utils.SharedPreferencesUtil;
...
public class NjjBleManger {
    ...
    public void init(Context context) {
        ...
        request = new SearchRequest.Builder()
                .searchBluetoothLeDevice(1000 * 40, 1)   // 先扫BLE设备1次，每次20s
                .build();
    }
...
//                .searchBluetoothLeDevice(1000 * 20, 1)   // 先扫BLE设备1次，每次20s
    ...
    public void unregisterConnectStatue() {
        unRegisterConnection(mac,statusChangeListener);
    }
    ...
    public void writeDailRuiYu(byte[] command, BleWriteResponse response) {
        bluetoothClient.writeNoRsp(mac, UUIDConfig.SERVICE_UUID_NJY, UUIDConfig.WRITE_UUID_NJY,
                command, response);
    }
    ...
    public void connectionRequest(BLEDevice bleDevice) {
        ...
        if (!id.startsWith(NJYID)){
            ConnectStatuesCallBack.onConnectFail(100);
        }else {
...
//                        ConnectStatuesCallBack.onConnected(bleDevice.getDevice().getAddress());
                        ...
                        BLESDKLocalData.getInstance().setMacAddress(mac);
                    }else {
                        ConnectStatuesCallBack.onConnectFail(i);
                    }
                }
            });
        }
...
//    ConnectStatusListener connectStatusListener;
    ...
    public void discoveredServices(int code) {
        ConnectStatuesCallBack.onDiscoveredServices(code);
    }
