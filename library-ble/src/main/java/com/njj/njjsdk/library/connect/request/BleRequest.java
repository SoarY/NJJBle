...
import com.njj.njjsdk.library.connect.BleConnectWorker;
import com.njj.njjsdk.library.connect.IBleConnectDispatcher;
import com.njj.njjsdk.library.connect.IBleConnectWorker;
...
//        LogUtil.e("add writeCharacteristicWithNoRsp 0");
        checkRuntime();

        BleConnectWorker worker = (BleConnectWorker) mWorker;

        BluetoothGattCharacteristic characteristic = worker.getCharacter(service, character);
        ...
        if (worker.getBluetoothGatt() == null) {
            BluetoothLog.e(String.format("ble gatt null"));
            return false;
        }

//        LogUtil.e("add writeCharacteristicWithNoRsp 1");
        characteristic.setValue(value != null ? value : ByteUtils.EMPTY_BYTES);
        characteristic.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);

        if (!worker.getBluetoothGatt().writeCharacteristic(characteristic)) {
            BluetoothLog.e(String.format("writeCharacteristic failed"));
            return false;
        }
//        LogUtil.e("add writeCharacteristicWithNoRsp 2");
        return true;
    }

    public BluetoothGatt getBluetoothGatt(){
        return mWorker.getBluetoothGatt();
    }
