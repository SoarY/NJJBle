...
class OtaActivity : AppCompatActivity() {
    ...
    fun push_ota(view: android.view.View) {
        if (selectPath==null){
            ...
            NJJOtaManage.getInstance().dfuUpdate(NjjBleManger.getInstance().mac, selectPath,object :NjjOtaCallBack{
                override fun onSuccess() {
                    tv_info.text = "升级成功"
                }

                override fun onError(errorType: Int, errorCode: Int) {
                    tv_info.text = "升级失败：errorType=$errorType errorCode=$errorCode"
                }

                override fun onProgressChanged(progress: Int) {
                    tv_info.text="$progress%"
                }
            })
        }
